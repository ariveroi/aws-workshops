  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    taskrole.addManagedPolicy(
      iam.ManagedPolicy.fromManagedPolicyArn(
        this,
        "ecs-secrets-manager-iam-policy",
    ...
    // Create Amazon RDS
    const instanceIdentifier = "awsomebuilder-rds";
    const credsSecretName = `awsomebuilder/rds/creds/${instanceIdentifier}`;
    const creds = new DatabaseSecret(this, "mysqlRdsCredentials", {
      secretName: credsSecretName,
      username: "admin",
    });

    const dbServer = new DatabaseInstance(this, "AwsomeBuilderRDS", {
      vpcSubnets: {
        onePerAz: true,
        subnetType: SubnetType.PRIVATE_ISOLATED,
      },
    ...
    const readReplica = new DatabaseInstanceReadReplica(
      this,
      "AwsomeBuulderReadReplica",
      {
        sourceDatabaseInstance: dbServer,
        vpc: vpc,
        instanceType: InstanceType.of(InstanceClass.T2, InstanceSize.LARGE),
      }
    );
